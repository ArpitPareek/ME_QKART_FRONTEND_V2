...
const Checkout = () => {
  ...
  const getAddresses = async (token) => {
    if (!token) {
      enqueueSnackbar("You must be logged in to access checkout page", { vairant: "warning" });
      return
    };
  ...
  const validateRequest = (items, addresses) => {
    ...
    const newBalance = `${balance - getTotalCartValue(items)}`;
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      enqueueSnackbar('Could not checkout', {
        variant: "error"
      })
      return false;
    }
  ...
  const OrderDetails = () => {
    return (
      <Box className="cart">
        <Typography color="#3C3C3C" variant="h4" sx={{ fontWeight: 'bold', pl:1, pt: 2 }} >
          Order Details
        </Typography>
        <Box 
        display="flex"
        ...
        alignItems="center"
        >
            <Typography color="#3C3C3C" variant="body1" sx={{ fontWeight: 'bold', pl:1, pt:2 }} >
              Products
            </Typography>
            <Typography color="#3C3C3C" variant="body1" sx={{ fontWeight: 'bold', pr:1, pt:2 }} >
              {getTotalItems(items)}
            </Typography>
        </Box>
        <Box 
        ...
        alignItems="center"
        >
            <Typography color="#3C3C3C" variant="body1" sx={{ fontWeight: 'bold', pl:1, pt:2, pb:2 }} >
              Shipping Charges
            </Typography>
            <Typography color="#3C3C3C" variant="body1" sx={{ fontWeight: 'bold', pr:1, pt:2, pb:2 }} >
              $0
            </Typography>
        </Box>
      </Box>
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               {(addresses.length > 0) && <Typography my="1rem">
               ...
               </Typography>}
               {
                 addresses.all.map((address)=>{
                  return (
                    ...
                    >
                        <Typography color="#3C3C3C" variant="body1" sx={{ fontWeight: 'bold' }} >
                          {address.address}
                        </Typography>
                        <Typography color="#3C3C3C" variant="body1" sx={{ fontWeight: 'bold'}} >
                        <Button variant="text" startIcon={<Delete />} onClick={(e)=>{
                          e.stopPropagation();
            ...
            <Button
              ...
              onClick={()=>{
                if (validateRequest(items, addresses)){
                  if (performCheckout(token, items, addresses)){
                    history.push("/thanks");
                  }
                }
              }}
